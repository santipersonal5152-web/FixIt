...
export default function ServidorPlomero() {
  ...
  const [tarjetasEmergencia, setTarjetasEmergencia] = useState([
    {
      id: "emergencia-1",
      titulo: "Emergencias urgentes",
      tarjetaClienteProps: {
        nombre: "Maria lucero",
        descripcion: "Fuga de agua grave",
        direccion: "Calle 123 #45-67",
        tiempo: "Hace 2 min",
        urgencia: "URGENTE - Inmediato",
        destino: "/detallesserviciopro",
        parametros: { emergenciaId: "urgente-1", prioridad: "alta" },
        id: "cliente-emergencia-1"
      }
    }
  ]);
  ...
  return (
    <View style={{ flex: 1, backgroundColor: "#8CB3DB" }}>
      ...
      >
        ...
        {/* Renderizar tarjetas de emergencia dinámicamente */}
        {tarjetasEmergencia.map((emergencia) => (
          <View key={emergencia.id} style={{ marginBottom: 30 }}>
            <TarjetaEmergencia
              titulo={emergencia.titulo}
              tarjetaClienteProps={{
                ...emergencia.tarjetaClienteProps,
                onAceptar: manejarAceptar,
                onRechazar: manejarRechazar,
              }}
              onRemove={eliminarTarjetaEmergencia}
              id={emergencia.id}
            />
          </View>
        ))}
