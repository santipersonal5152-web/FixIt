...
export default function TarjetaClienteAceptada({
  ...
  destino, // Ruta a la que navegará el botón (opcional)
  parametros = {}, // Parámetros para la navegación
  onRemove // Función para eliminar la tarjeta
}) {
  const router = useRouter();

  const handleCancelar = () => {
    // Primero ejecuta la función onCancelar si existe
    if (onCancelar) {
      onCancelar(id);
    }
    
    // Luego ejecuta la función de eliminación si existe
    if (onRemove) {
      onRemove(id);
    }
    
    // Finalmente navega si hay un destino especificado
    if (destino) {
  ...
  return (
    <View style={styles.card}>
      ...
      {/* Sección inferior: botón único */}
      <View style={styles.bottomRow}>
        <TouchableOpacity 
          style={styles.botonCancelar}
          onPress={handleCancelar}
        >
          <Text style={styles.textoBoton}>Cancelar servicio</Text>
        </TouchableOpacity>
      </View>
    </View>
  );
